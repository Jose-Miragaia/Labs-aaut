...
% escolhemos o 6 pq é o de menor ordem que tem o menor erro
...
% NOW LEGIT
% o número de vectores de suporte para a criação do SVM é sempre
% elevado,sendo que para o caso polinomial vai ser dificil conseguir
% adaptar limites sem usar muito vectores de suporte
% logo não sei o que dizer, isto é aborrecido hi, how are u
% 
...
% mais elaborado em torno de cada vector, sendo utilizados todos os vectores como vectores de suporte pois nestas situações 
% o agloritmo vai tentar considerar cada um dos vectores como individual, fazendo que não exista erro nos vectores de treina no entanto muito pouco flexivel
% mais concretamente ocorre overfitting. à medida que se vai aumentado o
% simga consegue-se notar que se vai criando uma área que engloba os
% datassets , sendo cada vez menos rigida até a um ponto em que começa a
% englobar pontos pertencentes ao outro (sinal)

% Como existiam vários valores em que o erro é o nos escolhemos o valor de
...
% pertencentes à mesma classe criou bubbles em volta de um dos outleirs
