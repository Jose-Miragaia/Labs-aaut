	...
	while 1			% Main loop: Read the input text and then classify it
		...
		for languageindex = 1:numel(languages)          % Loop through the available languages
			...
			for trigramindex = 1:numel(text)-2;					% Loop through all the trigrams of the input text
				...
				%
                %appearances(trigramindex) =log(trigocurrences/totallanguageocurrences)
                %its added +1 to every trigram to use LaPlacian Smoothing
                %this obtains probability of trigram given the language and
                %logs it
                 appearances(trigramindex) = log(trigramcount+1) - log(double(total_counts(languageindex)+(total_counts(languageindex)+counts{1}.Count)));
			...
			%
            %its summed to logarithms of P(language) and P(tri1|language)
            %and  P(tri2|language)
			scores(languageindex) = sum(appearances) + (Prob_lang(languageindex));
           %scores(languageindex) = log(double(prod(appearances) *Prob_lang(languageindex)));
			%
